...
**tool_cache.py** — `CachedTool`, an `AgentTool` wrapper that serves a tool's result from cache when the same input was seen before. Wrap only tools whose result is a function of input and scope — a tool with side effects must run every time. The key is a SHA-256 over the tool name, the model's input, and a `scope` mapping for whatever else the result depends on. Two tiers: `ToolCache`, a `cachetools` `TLRUCache` bounded by entries and encoded bytes with a jittered TTL (`get_tool_cache()` returns the process-wide instance, so it outlives any one env), and optionally `RayToolCache`, the client for `ToolCacheActor` — a named Ray actor shared by every process of the job (`get_shared_tool_cache()`) that also runs each key once job-wide: the first caller owns the run (`reserve` → `insert`/`release`), later callers block in `wait` until it ends. In-process, a class-level `inflight` table coalesces concurrent calls for the same key, and a shared-tier hit is copied into the local tier. `on_access` receives one `ToolCacheAccess` per call (outcome hit/miss/coalesced, tier reached, latency, key) for metrics. Wiring is per env — take `tool_cache`/`shared_tool_cache` as named `__init__` params and wrap in `get_tools()`; no shipped env wires it yet. The Ray tier needs `ray` installed.
