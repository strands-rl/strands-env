...
**harbor/** — `HarborEnv` runs any Harbor-format task (local Docker or self-hosted e2b sandbox) for task execution. `HarborConfig` extends `EnvironmentConfig` with `task_id`, `task_dir`, `trial_dir`, `harbor_env_config`, `timeout`, `backend`, `e2b_backend_config`. Provides `execute_command` tool for shell commands in the container. `HarborReward` runs verification tests (`tests/test.sh`) in the container for binary (0/1) reward. Both the `terminal-bench-*` and `swebench-verified` eval benchmarks run on this single env — they differ only in dataset and system prompt (the SWE-bench evaluator injects its prompt via the serializable `system_prompt` config key, so there is no separate SWE env class).
